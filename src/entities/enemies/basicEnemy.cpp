...
basicEnemy::basicEnemy(int localID, int playerLevel, sf::Vector2f playerPos, entitiesManager* entitiesManager) : baseEntity(localID) {
    ...
    this->health = darkMath::getInstance().generateFloatNumber(50.f+(70.f*level*0.4f), 100.f+(70.f*level*0.4f));
    this->maxHealth = this->health;
    this->isAlive = true;
    this->EXP = darkMath::getInstance().generateFloatNumber(10.f+(30.f*level*0.2f), 40.f+(30.f*level*0.2f));
